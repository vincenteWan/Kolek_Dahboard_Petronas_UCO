...
function createAreaChart() {
    ...
    const data = {
        labels: ['Kuala Lumpur', 'Ampang', 'Shah Alam', 'Cheras'],
        datasets: [{
            data: [35, 25, 25, 15],
            backgroundColor: [
                '#4CAF50',
...
function handleRefresh() {
    ...
    setTimeout(() => {
        ...
        if (window.ucoChart) {
            window.ucoChart.data.datasets[0].data = [
                65, 70, 75, 80, 85, 80, 68, 78, 62, 57
            ];
...
function handleYearChange(event) {
    ...
    // Update chart with new year data
    if (window.ucoChart) {
        // Simulate different data for different years
        const yearData = {
            2025: [65, 70, 75, 80, 85, 80, 65.7, 75, 60, 55],
            2024: [45, 50, 55, 60, 65, 70, 75, 70, 65, 60],
            2023: [30, 35, 40, 45, 50, 55, 60, 55, 50, 45]
        };
        
        window.ucoChart.data.datasets[0].data = yearData[year] || yearData[2025];
        window.ucoChart.update();
    }
}
// Handle month filter
...
(function(){
    ...
    function initStations() {
        ...
        loadStationFromCSV().then(stations => {
    ...
    function renderStationTable(page = currentPage) {
        ...
        for (let i = start; i < end; i++) {
            ...
            let actionContent = '';
            // If station is full (danger), show an '[Empty bin]' action highlighted in red (design-only)
            if (r.status === 'danger') {
                ...
                // Not full - show a subtle '[Empty bin]' action
            ...
            tr.innerHTML = `
                ...
                <td class="map-col"><a href="#" class="view-map">View map <i class="fas fa-location-arrow"></i></a></td>
                <td>${r.fill}%</td>
                <td>${r.purity}%</td>
    ...
    function applyStationFilters() {
        ...
        filteredData = stationData.filter(s => {
            const textMatch = !q || (
                ...
                String(s.purity || '').toLowerCase().includes(q)
            );
            ...
            exportBtn.addEventListener('click', function() {
                ...
                window.dashboardAPI.exportStations({ filteredIds: filteredData.map(s => s.id) }).then(resp => {
                    if (resp && resp.success) {
                        ...
                        // For now, do client-side CSV generation as fallback
                        const rows = filteredData.map(r => 
                            [r.id, r.name, r.fill + '%', r.purity + '%', statusText(r.status)].join(',')
                        ).join('\n');
                        const csv = 'Station ID,Station Name,Fill Level,Purity Level,Status\n' + rows;
                        const blob = new Blob([csv], { type: 'text/csv' });
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = 'stations_export.csv';
                        a.click();
    ...
    function attachRowHandlers() {
        ...
        emptyLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                e.preventDefault();
                // Open the Pending Collection modal (matches screenshot)
                const stationId = this.dataset.stationId;
                const stationName = this.dataset.stationName || '';
                const fill = this.dataset.fill || '';
                openPendingCollectionModal(stationId, stationName, fill);
            });
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Collector page interactions: load collectors from CSV then setup UI
...
function applyHouseholdFilters() {
    ...
    householdFilteredData = householdFullData.filter(d => {
        const status = normalizeStatusText(d.accountStatus || d.status);
        const isInactive = status.includes('inactive') || status.includes('deactiv');
        const isActive = status.includes('active') && !isInactive;
...
function applyCollectionFilters() {
    ...
    collectionFilteredData = collectionFullData.filter(d => {
        const status = normalizeStatusText(d.collectionStatus);
...
function setupCollectorInteractions() {
    ...
    const filterSelect = document.getElementById('collector-filter-select');
    if (search && table) {
    ...
    if (addBtn) addBtn.addEventListener('click', e => { e.preventDefault(); alert('Add New Collector — form not implemented (design-only)'); });
    ...
    if (exportBtn) exportBtn.addEventListener('click', e => {
        ...
        try {
            ...
            const header = ['Collector ID','Collector Name','IC Details','Phone Number','Email Address','Vehicle','Plate No.','Preferred Area','Availability'];
            const csvRows = [header.join(',')];
            rows.forEach(r => {
                const cells = Array.from(r.querySelectorAll('td'));
                const dataCells = cells.slice(0, 9);
            ...
            window.dashboardAPI.exportCollectors({ count: rows.length }).then(resp => {
                ...
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a'); a.href = url; a.download = 'collectors_export.csv'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    ...
    if (detailsExport) detailsExport.addEventListener('click', e => { e.preventDefault(); alert('Export PDF — design-only'); });
}
...
// Pending Collection Modal (opened from Empty Bin action)
function openPendingCollectionModal(stationId, stationName, fill) {
    ...
    if (!modal) return;
    // Mock pending collections data for demo
    const pendingCollections = [
        { id: '#CR-050', household: '#H-058', location: '2, Jalan SS15, Subang Jaya', time: '4:15 PM' }
    ];
    const tbody = modal.querySelector('#pending-collection-tbody');
    if (tbody) {
        tbody.innerHTML = '';
        pendingCollections.forEach(pc => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${escapeHtml(pc.id)}</td>
                <td>${escapeHtml(pc.household)}</td>
                <td>${escapeHtml(pc.location)}</td>
                <td>${escapeHtml(pc.time)}</td>
                <td><select class="assign-select"><option>Select Collector</option><option>Mohd. Zafir Ali</option><option>Mohd. Daud Raj</option><option>Daniel Tan</option><option>Mohd. Ahmad Raj</option><option>Mohd. Hamzah Syah</option><option>Mariusis Jaths</option></select></td>
                <td><a href="#" class="ai-suggestion-link">AI Suggestion →</a></td>
            `;
            tbody.appendChild(tr);
        });
    }
    const entriesInfo = modal.querySelector('#pending-entries-info');
    if (entriesInfo) entriesInfo.textContent = `Showing 1 to 1 of 1 entries`;
    modal.classList.add('show');
...
function openCollectorDetails(data = {}) {
    ...
    try {
        ...
        if (nameEl) {
            // nameEl may contain child nodes; set text node only
            if (nameEl.firstChild) nameEl.firstChild.textContent = data.name || nameEl.firstChild.textContent;
            else nameEl.textContent = data.name || nameEl.textContent;
        }
        if (idEl) idEl.textContent = data.id || idEl.textContent;
    ...
    function onKey(e) {
        if (e.key === 'Escape' || e.key === 'Esc') {
            ...
            // will be cleaned in closeCollectorDetails via stored reference
        }
    }
    document.addEventListener('keydown', onKey);
    // keep reference so closeCollectorDetails can remove it later
    modal._onKey = onKey;
}
...
function setupCollectionInteractions() {
    ...
    if (exportBtn) exportBtn.addEventListener('click', e => {
        ...
        try {
            ...
            rows.forEach(r => {
                const cells = Array.from(r.querySelectorAll('td'));
                const dataCells = cells.slice(0,11);
                const values = dataCells.map(c => '"' + c.textContent.trim().replace(/"/g,'""') + '"');
                csvRows.push(values.join(','));
            });
            const csv = csvRows.join('\n');
            window.dashboardAPI.exportCollections({ count: rows.length }).then(resp => {
                const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a'); a.href = url; a.download = 'collections_export.csv'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    ...
    // view details -> open collector details modal (use collector ID to find name if available)
    document.querySelectorAll('.collection-table .view-details').forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            const row = this.closest('tr');
            if (!row) return;
            const collectorId = (row.querySelector('td:nth-child(2)') && row.querySelector('td:nth-child(2)').textContent || '').trim();
            let name = '';
            // try to find collector name in collector table
            const collectors = document.querySelectorAll('.collector-table tbody tr');
            collectors.forEach(r => {
                const idCell = r.querySelector('td:nth-child(1)');
                const nameCell = r.querySelector('td:nth-child(2)');
                if (idCell && idCell.textContent.trim() === collectorId) {
                    name = nameCell ? nameCell.textContent.trim() : '';
                }
            });
            if (!name) name = 'Collector';
            openCollectorDetails({ id: collectorId, name });
        });
    });
}
function setupHouseholdInteractions() {
    ...
    // pagination buttons: basic UI toggle
    ...
    if (exportBtn) exportBtn.addEventListener('click', e => {
        ...
        try {
            ...
            rows.forEach(r => {
                const cells = Array.from(r.querySelectorAll('td'));
                // ignore actions cell (last)
                const dataCells = cells.slice(0, 9);
                const values = dataCells.map(c => '"' + c.textContent.trim().replace(/"/g, '""') + '"');
                csvRows.push(values.join(','));
            });
            const csv = csvRows.join('\n');
            // Call backend (mock) then trigger client-side download (fallback)
...
function populateHouseholdTableFromData(data) {
    ...
    const rows = data.map(d => {
        ...
        const action = (String(status).toLowerCase().includes('active')) ? `<a href="#" class="danger-action">[Deactivate]</a>` : `<a href="#" class="success-action">[Reactivate]</a>`;
        return `
            <tr>
                <td>${escapeHtml(id)}</td>
...
function renderApplicantTableFromData(data) {
    ...
    const rows = data.map(d => {
        ...
        const action = `<button class="action-accept btn-small" title="Approve"><i class="fas fa-check-circle" style="color:#2e9f69"></i></button>
                        <button class="action-reject btn-small" title="Reject"><i class="fas fa-times-circle" style="color:#e74c3c"></i></button>`;
        return `<tr>${cells}<td class="actions-col">${action}</td></tr>`;
    }).join('\n');
...
function setupReportInteractions() {
    ...
    if (filterBtn && filterSelect) {
        ...
        filterBtn.addEventListener('click', function(e) {
            ...
            const labels = { all: 'All', pending: 'Pending Review', reviewed: 'Reviewed' };
    ...
    if (refreshBtn) refreshBtn.addEventListener('click', function(e) {
        ...
        console.log('Report refresh (demo)');
    });
    // Export button -> CSV fallback
    ...
    // Report 'View details' -> open report modal (accident report view)
    document.querySelectorAll('.report-table .view-details').forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            const row = this.closest('tr');
            if (!row) return;
            const reportId = (row.querySelector('td:nth-child(1)') && row.querySelector('td:nth-child(1)').textContent || '').trim();
            const collectorId = (row.querySelector('td:nth-child(2)') && row.querySelector('td:nth-child(2)').textContent || '').trim();
            const dateTime = (row.querySelector('td:nth-child(3)') && row.querySelector('td:nth-child(3)').textContent || '').trim();
            const reportType = (row.querySelector('td:nth-child(4)') && row.querySelector('td:nth-child(4)').textContent || '').trim();
            // Report-specific mapping
            if (reportType.toLowerCase().includes('accident')) {
                // Mock mapping for demonstration - more rows can be added
                let details = {
                    id: reportId,
                    type: reportType,
                    collector: collectorId,
                    date: dateTime,
                    location: '3, Jalan SS18/12B, Petaling Jaya',
                    accidentType: 'Minor Vehicle Damage',
                    description: 'I was hit by another car at the junction. No injuries.'
                };
                openReportDetails(details);
            } else if (reportType.toLowerCase().includes('collection')) {
                // Collection report -> populate fields used for collection reports
                let cdetails = {
                    id: reportId,
                    type: reportType,
                    collector: collectorId,
                    date: dateTime,
                    collectionId: '#CR-087',
                    missReason: 'Household Not Home',
                    description: 'Arrived at location but no one was available. Attempted to call twice.'
                };
                openReportDetails(cdetails);
            } else if (reportType.toLowerCase().includes('manual')) {
                ...
                const mdetails = {
                    ...
                    collector: collectorId,
                    date: dateTime,
                    title: 'Vehicle requires maintenance check',
                    category: 'Vehicle/Maintenance',
                    description: "Noticed a rattling noise from the engine during today's route."
                };
                openReportDetails(mdetails);
            } else {
                // generic report type - simple view
                openReportDetails({ id: reportId, type: reportType, collector: collectorId, date: dateTime, location: '-', accidentType: '-', description: '-' });
            }
        });
    });
...
function openReportDetails(data = {}) {
    ...
    if (!modal) return;
    try {
        if (data.id) document.getElementById('rd-report-id').textContent = data.id;
        if (data.type) document.getElementById('rd-report-type').textContent = data.type;
        if (data.collector) document.getElementById('rd-collector-id').textContent = data.collector;
        if (data.date) document.getElementById('rd-date').textContent = data.date;
        if (data.location) document.getElementById('rd-location').textContent = data.location;
        if (data.accidentType) document.getElementById('rd-accident-type').textContent = data.accidentType;
        if (data.collectionId) document.getElementById('rd-collection-id').textContent = data.collectionId;
        if (data.missReason) document.getElementById('rd-miss-reason').textContent = data.missReason;
        if (data.title) document.getElementById('rd-title').textContent = data.title;
        if (data.category) document.getElementById('rd-category').textContent = data.category;
        if (data.description) document.getElementById('rd-description').textContent = data.description;
    } catch (e) { /* ignore */ }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const markBtn = document.getElementById('report-mark-resolved');
    if (markBtn) markBtn.addEventListener('click', e => { e.preventDefault(); alert('Marked as resolved (demo)'); closeReportDetails(); });
    const contactBtn = document.getElementById('report-contact-collector');
...
function setupSettingsInteractions(){
    ...
    // load saved state
    try {
        if (themeToggle && localStorage.getItem('settings:theme') === '1') themeToggle.checked = true;
    ...
    if (themeToggle) themeToggle.addEventListener('change', function(){
        try{ localStorage.setItem('settings:theme', this.checked ? '1' : '0'); }catch(e){}
    });
    ...
    if (lang) lang.addEventListener('change', function(){ try{ localStorage.setItem('settings:language', this.value); }catch(e){} });
    ...
    if (rformat) rformat.addEventListener('change', function(){ try{ localStorage.setItem('settings:reportFormat', this.value); }catch(e){} });
...
function setupAIAssist() {
    ...
    function sendPrompt(){
        ...
        // design-only: show a small toast + clear
        console.log('Prompt sent to KolekBot:', val);
        alert('KolekBot receives: "' + val + '" (demo)');
        input.value = '';
    }
    ...
    const rows = pageData.map(d => {
        ...
        return `
            <tr>
                <td>${escapeHtml(id)}</td>
                <td>${escapeHtml(name)}</td>
                <td><a href="#" class="view-ic">${ic}</a></td>
                <td>${escapeHtml(phone)}</td>
                <td>${escapeHtml(email)}</td>
...
function renderCollectionPage(page = 1) {
    ...
    const rows = pageData.map(d => {
        const collectionId = displayVal(d.collectionID);
        const date = displayVal(d.collectionDate, v => new Date(v).toLocaleDateString('en-GB'));
        const address = displayVal(d.collectionAddress);
        const requestTime = displayVal(d.requestTime);
        const collectedTime = displayVal(d.collectedTime);
        const status = displayVal(d.collectionStatus);
        const points = displayVal(d.pointsEarned);
        const credit = displayVal(d.creditEarned);
        const householdId = displayVal(d.householdID);
        const collectorId = displayVal(d.collectorID);
        const stationId = displayVal(d.stationID);
        ...
        return `
            <tr>
                ...
                <td class="actions-col"><a href="#" class="view-details">[View details]</a></td>
            </tr>
        `;
    }).join('\n');
    tbody.innerHTML = rows;
...
function updateCollectionKPI() {
    ...
    try {
        ...
        if (pendingBadge) {
            const pendingCount = collectionFullData.filter(d => String(d.collectionStatus).toLowerCase().includes('pending')).length;
